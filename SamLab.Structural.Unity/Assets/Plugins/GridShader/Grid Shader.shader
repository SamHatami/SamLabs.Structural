Shader "Ogxd/Grid"
{
   Properties
   {
      _Color("Main Color", Color) = (0.5, 1.0, 1.0, 0.7)
      ...
      _PrimaryAlpha("Primary Line Alpha", Range(0.0, 1.0)) = 0.7
      _SecondaryAlpha("Secondary Line Alpha", Range(0.0, 1.0)) = 0.4
   }
   SubShader
   {
      ...
      Pass
      {
         ...
         fixed4 frag (v2f i) : SV_Target
         {
            ...
            // Set background color as the default
            col = _BackgroundColor;

            // Check for primary grid lines
            if (pos.x == 1 || pos.y == 1) {
               col = _Color;
               // Use configured alpha for primary lines, adjusted by fade transition
               col.a = _PrimaryAlpha * max((1 - fade), fade);
            } else {
               // Check for secondary grid lines
               pos.x = applyScale(i.uv.x, 10.0 * localScale);
               pos.y = applyScale(i.uv.y, 10.0 * localScale);

               if (pos.x == 1 || pos.y == 1) {
                  col = _SecondaryColor;
                  // Use configured alpha for secondary lines, adjusted by fade transition
                  col.a = _SecondaryAlpha * (1 - fade);
               }
            }
